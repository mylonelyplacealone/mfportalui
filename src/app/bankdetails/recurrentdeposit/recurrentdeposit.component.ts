...
@Component({
  selector: 'app-recurrentdeposit',
  templateUrl: './recurrentdeposit.component.html',
  styleUrls: ['./recurrentdeposit.component.css', '../common/bank.css']
})
export class RecurrentdepositComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.bnkSer.GetAccountList("RECURRENTDEPOSIT");
    this.message = "";
  }
  ...
  openDialog(record:Account, mode:string){
    ...
    let dialogRef = this.dialog.open(AccounteditComponent, {
      width: '700px',
      data:{ record, mode, accType: "RECURRENTDEPOSIT"}, 
    });
